...
Publica:
    - cmd.cancel_fabrication → Warehouse
    - cmd.refund → Payment

Consume:
    - evt.fabrication_canceled ← Warehouse
...
# region Publishers
async def publish_cancel_fabrication_command(order_id: int, saga_id: str):
...
# region Consumers
async def _handle_evt_fabrication_canceled(message):
    """Traduce evt.fabrication_canceled a evento interno del CancelSaga."""
    async with message.process():
        data = json.loads(message.body)
        saga_id = data.get("saga_id")
        if not saga_id:
            logger.warning("[ORDER] evt.fabrication_canceled sin saga_id: %s", data)
            return
...
async def listen_evt_fabrication_canceled():
    """Escucha confirmación de cancelación desde Warehouse."""
    _, channel = await get_channel()
    exchange = await declare_exchange(channel)

    queue = await channel.declare_queue("evt_fabrication_canceled_queue", durable=True)
